...
class IndividualEvents:
    ...
    def random_have_child(self):
        if random.random() < 1 and 'spouse' in self.character.relationships and self.character.gender == 'female':
            spouse = self.model.characters[self.character.relationships['spouse']]
            if random.random() < 1 and spouse.relationships['spouse'] == self.character.id:
