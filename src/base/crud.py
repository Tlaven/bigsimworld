...
def update_multiple_characters(session, characters: dict[int: object]):
    # 构建批量更新的字典列表
    updates = [
        {key: value for key, value in character.__dict__.items() if key in Table.__table__.columns.keys()} for character in characters.values()
    ]
    # 使用 bulk_update_mappings 进行批量更新
    session.bulk_update_mappings(Table, updates)
    updated_count = len(updates)
    logger.info(f"{updated_count} characters updated successfully.")
...
def get_characters(session, filters=None, ids=None):
    try:
